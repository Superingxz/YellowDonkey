package com.ruanjie.donkey.ui.webView;
...
import com.ruanjie.donkey.ui.webView.presenter.WebViewPresenter;
...
public class WebViewActivity extends ToolbarActivity<WebViewPresenter> implements WebViewContract.View {
    ...
    public static void start(Context context, String title, String url,int messageId,int couponId) {
        Intent starter = new Intent(context, WebViewActivity.class);
        starter.putExtra(KEY_TITLE, title);
        starter.putExtra(KEY_DATA, url);
    ...
    @BindView(R.id.bt_get_coupons)
    AppCompatButton btGetCoupons;

    WebView mWebView;

    @OnClick(R.id.bt_get_coupons)
    void onGetCoupons(){
        getPresenter().getCoupon(messageId,couponId);
    }
    ...
    protected void initialize() {
        ...
        loadUrl(url);
        if (couponId > 0){
            btGetCoupons.setVisibility(View.VISIBLE);
        }else {
            btGetCoupons.setVisibility(View.GONE);
        }
    }
...
//        webSettings.setGeolocationDatabasePath(dir);
    ...
    public void getCouponSuccess() {
        ToastUtil.s("领取成功");
        btGetCoupons.setBackground(ContextCompat.getDrawable(getContext(),R.drawable.shape_radius_gray_1024dp));
        btGetCoupons.setText("已领取");
    }
