package com.ruanjie.donkey.ui.shop.contract;

import com.ruanjie.donkey.bean.BuyGoodsBean;
import com.ruanjie.donkey.bean.GoodsDetailBean;
import com.ruanjie.donkey.bean.UploadBean;
import com.ruanjie.donkey.bean.UploadBean2;
import com.softgarden.baselibrary.base.IBaseDisplay;
import com.softgarden.baselibrary.base.IBasePresenter;

/**
 * 项目名:   YellowDonkey
 * 包名:     com.ruanjie.donkey.ui.main.contract
 * 文件名:   ShopContract
 * 创建者:    QJM
 * 创建时间: 2019/8/13 10:15
 * 描述:     TODO
 */
public interface ShopAppraiceContract {

    interface View extends IBaseDisplay {

        void appraiceGoods(String data);

        void uploadImage2(UploadBean2 content);

    }

    interface Model extends IBasePresenter {

        void appraiceGoods(int order_id, int star, String content, int is_anonymous, String images);

        void uploadImage2(String content);


    }
}
